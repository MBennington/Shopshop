  ...
  const fetchWallet = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const data = await response.json();
        const errorMessage =
          data.msg || data.error || 'Failed to load wallet information';
  ...
  const fetchPayouts = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const data = await response.json();
        const errorMessage =
          data.msg || data.error || 'Failed to load payout history';
        toast.error(errorMessage);
      }
    } catch (error) {
  ...
  const handleCreatePayout = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errorMessage =
          data.msg || data.error || 'Failed to create payout request';

        // Check if error is about missing bank details
        if (
          errorMessage.toLowerCase().includes('bank details') ||
          errorMessage.toLowerCase().includes('bank details not found')
        ) {
          toast.error('Bank details required', {
            description: errorMessage.includes('Settings')
              ? errorMessage
              : 'Please fill in your bank details in Settings > Payout before making a payout request.',
            action: {
  ...
  const handleCancelPayout = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/payout/${cancelDialog.payoutId}/cancel`,
        {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (response.ok) {
        ...
        const errorMessage =
          data.msg || data.error || 'Failed to cancel payout';
        toast.error(errorMessage);
      }
    } catch (error: any) {
  ...
  return (
    <div className="relative flex size-full min-h-screen flex-col bg-white group/design-root overflow-x-hidden">
                ...
                onClick={() => {
                  if (!wallet || wallet.available_balance <= 0) {
                    toast.warning('No available balance', {
                      description:
                        'You need to have available balance to request a payout.',
                    });
                    return;
                  }
                ...
                <div className="rounded-xl border border-[#dde0e3] bg-white p-4">
                  ...
                  <div className="space-y-4">
                    {pendingPayouts.length === 0 ? (
                      <p className="text-sm text-[#6a7581] text-center py-4">
                        No active payouts
                      </p>
                    ) : (
                        ...
                        >
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              <span
                                className={`px-2 py-1 rounded text-xs font-medium ${getStatusColor(payout.status)}`}
                              >
                                {payout.status}
                              </span>
                            </div>
      ...
      {showRequestDialog && (
        <>
          <div
            className="fixed inset-0 bg-white/60 backdrop-blur-sm z-[9998]"
            onClick={() => {
              setShowRequestDialog(false);
              setRequestAmount('');
            }}
      ...
      {cancelDialog.show && (
        <>
          <div
            className="fixed inset-0 bg-white/20 backdrop-blur-sm z-[9998]"
            onClick={() => {
              setCancelDialog({ show: false, payoutId: null });
            }}
          />
          <div className="fixed inset-0 flex items-center justify-center z-[9999]">
            <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4 shadow-xl border border-gray-200">
              ...
              <p className="text-sm text-[#6a7581] mb-4">
                Are you sure you want to cancel this payout request? The amount
